...
void ARogueRocketProjectile::Explode()
{
	...
	ARogueImpactEffect::SpawnImpactEffect(World, ExplosionLocation, FRotator::ZeroRotator, ERogueImpactVisualStyle::Explosion, ExplosionScale, VisualConfig.ImpactEffectLifetime, this);

	ARogueGameMode* RogueGameMode = World->GetAuthGameMode<ARogueGameMode>();
	const bool bCullCombatEffects = RogueGameMode != nullptr && RogueGameMode->ShouldCullCombatEffects();
	static UParticleSystem* ExplosionBurst = LoadObject<UParticleSystem>(nullptr, TEXT("/Game/GameAssets/Realistic_Starter_VFX_Pack_Vol2/Particles/Explosion/P_Explosion_Big_A.P_Explosion_Big_A"));
	if (!bCullCombatEffects && ExplosionBurst != nullptr)
	{
		UGameplayStatics::SpawnEmitterAtLocation(World, ExplosionBurst, FTransform(FRotator::ZeroRotator, ExplosionLocation, FVector(0.52f, 0.52f, 0.52f)));
	}

	TArray<ARogueEnemy*> AffectedEnemies;
	TArray<float> PendingDamages;
	if (!IsValid(RogueGameMode))
	{
		DeactivateToPool();
		return;
	}
	...
	RogueGameMode->CollectEnemiesInRange(ExplosionLocation, ExplosionRadius, NearbyEnemies, 0, false);
