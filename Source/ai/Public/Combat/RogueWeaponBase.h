...
struct AI_API FRogueWeaponTableRow : public FTableRowBase
{
	...
	/** 基础伤害 */
	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "General", meta = (DisplayName = "基础伤害"))
	float Damage = 20.0f;

	/** 攻击间隔 / 冷却（秒） */
	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "General", meta = (DisplayName = "冷却时间"))
	float Cooldown = 1.0f;

	/** 索敌范围 */
	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "General", meta = (DisplayName = "索敌范围"))
	float Range = 1600.0f;

	/** 对应的"数量升级"枚举类型 */
	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "Upgrade", meta = (DisplayName = "数量升级类型"))
	ERogueUpgradeType CountUpgradeType = ERogueUpgradeType::ProjectileCount;

	// ---- 弹道随机化（弹体 & 火箭通用） ----

	/** 是否启用弹道随机平面偏转 */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "TrajectoryRandomization", meta = (DisplayName = "启用弹道偏转"))
	bool bEnableTrajectoryDeviation = false;

	/** 弹道偏转最大角度（度），实际偏转在 [-MaxAngle, +MaxAngle] 范围内随机 */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "TrajectoryRandomization", meta = (DisplayName = "弹道偏转最大角度", EditCondition = "bEnableTrajectoryDeviation", ClampMin = "0.0", ClampMax = "90.0"))
	float TrajectoryDeviationMaxAngle = 5.0f;

	/** 是否启用弹速随机增幅 */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "TrajectoryRandomization", meta = (DisplayName = "启用弹速随机"))
	bool bEnableSpeedRandomization = false;

	/** 弹速随机增幅比例（0~1），实际弹速 = 基础弹速 × (1 ± Ratio) */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "TrajectoryRandomization", meta = (DisplayName = "弹速随机比例", EditCondition = "bEnableSpeedRandomization", ClampMin = "0.0", ClampMax = "1.0"))
	float SpeedRandomizationRatio = 0.15f;

	// ---- 弹体武器专属 ----

	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "Projectile", meta = (DisplayName = "弹体类"))
	...
	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "Scythe", meta = (DisplayName = "刀片类"))
	TSubclassOf<ARogueOrbitingBlade> BladeClass;

	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "Scythe", meta = (DisplayName = "环绕半径"))
	float OrbitRadius = 220.0f;

	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "Scythe", meta = (DisplayName = "旋转速度"))
	float RotationSpeed = 130.0f;

	// ---- 火箭武器专属 ----

	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "Rocket", meta = (DisplayName = "火箭类"))
	TSubclassOf<ARogueRocketProjectile> RocketClass;

	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "Rocket", meta = (DisplayName = "爆炸半径"))
	float ExplosionRadius = 280.0f;

	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "Rocket", meta = (DisplayName = "火箭速度"))
	float RocketSpeed = 900.0f;

	// ---- 激光武器专属 ----

	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "Laser", meta = (DisplayName = "激光类"))
	TSubclassOf<ARogueLaserBeam> BeamClass;

	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "Laser", meta = (DisplayName = "折射次数"))
	int32 RefractionCount = 0;

	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "Laser", meta = (DisplayName = "折射范围"))
	float RefractionRange = 950.0f;

	// ---- 地狱塔武器专属 ----

	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "HellTower", meta = (DisplayName = "基础伤害(塔)"))
	float BaseDamage = 1.0f;

	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "HellTower", meta = (DisplayName = "每跳伤害递增"))
	float DamageRampPerTick = 0.5f;

	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "HellTower", meta = (DisplayName = "伤害跳间隔"))
	float DamageTickInterval = 0.10f;

	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "HellTower", meta = (DisplayName = "光束刷新间隔"))
	float BeamRefreshInterval = 0.16f;

	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "HellTower", meta = (DisplayName = "更新间隔"))
	float UpdateInterval = 0.05f;

	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "HellTower", meta = (DisplayName = "高负载更新间隔"))
	float HeavyCombatUpdateInterval = 0.08f;

	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "HellTower", meta = (DisplayName = "高负载光束刷新惩罚"))
